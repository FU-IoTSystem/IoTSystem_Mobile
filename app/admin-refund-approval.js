...
import { mockRefundRequests } from '../mocks';

export default function AdminRefundApprovalScreen() {
  const [items, setItems] = useState(mockRefundRequests);
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Refund Approvals</Text>
      <FlatList
        data={items}
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            ...
            <Row label="Original" value={`${item.originalAmount.toLocaleString()} VND`} />
            <Row label="Refund" value={`${item.refundAmount.toLocaleString()} VND`} />
